...
requested_git_ref="${9:-}"
receipt_path="${10:?receipt path required}"
...
exec /usr/local/libexec/kcml-install-repository-component \
  "$repository_key" "$source_commit" "$image_reference" "$image_digest" "$build_run_id" "$deploy_run_id" "$deploy_run_attempt" "$requested_git_ref" "$receipt_path"
