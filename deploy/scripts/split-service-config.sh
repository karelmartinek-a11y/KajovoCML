...
export BUILD_ID
export GENERATION_WORKER_ENABLED="false"
export MONITOR_ENABLED="true"
write_env "$root/web.env" \
  NODE_ENV PORT BUILD_ID CONFIG_VAULT_MASTER_KEY_ID PUBLIC_BASE_DOMAIN ADMIN_HOST AUTH_HOST REGISTER_HOST EGRESS_PROXY_SOCKET_PATH WILDCARD_TLS_CERT_PATH TRUSTED_PROXY_CIDRS CHROMIUM_BINARY LOG_LEVEL UI_TIME_ZONE

export GENERATION_WORKER_ENABLED="true"
export MONITOR_ENABLED="false"
write_env "$root/worker.env" \
  NODE_ENV BUILD_ID GENERATION_WORKER_ENABLED GENERATION_WORKER_INTERVAL_MS COMPONENT_WORKER_INTERVAL_MS GENERATION_ROOT GENERATION_SOURCE_ROOT GENERATED_COMPONENT_ROOT GENERATION_OPENAI_MODEL CHROMIUM_BINARY GENERATION_PRIVILEGED_HELPER SYSTEMCTL_BINARY PUBLIC_BASE_DOMAIN ADMIN_HOST AUTH_HOST REGISTER_HOST RUNTIME_SOCKET_ROOT EGRESS_PROXY_SOCKET_PATH LOG_LEVEL UI_TIME_ZONE CONFIG_VAULT_MASTER_KEY_ID
...
write_credential worker access_token_hmac "${ACCESS_TOKEN_HMAC_KEY_BASE64:-}"
