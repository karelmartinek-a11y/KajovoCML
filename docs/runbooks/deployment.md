...
- The production installer must preserve `kcml-deploy:kcml-deploy` ownership of `/opt/actions-runner/kcml-deploy/_work/_temp`; if that directory drifts to a root-owned state, GitHub Actions deployment jobs fail before checkout while preparing `_github_workflow`.
- That sudo policy allows `SETENV` only for the bounded wrapper commands and keeps only `PASS`, `GHCR_TOKEN`, `GHCR_ACTOR` and `KCML_FACTORY_RESET_CONFIRM`, so deployment can pass the minimum required secrets without opening broader environment injection.
...
3. Keep TLS SAN coverage for both `*.<PUBLIC_BASE_DOMAIN>` and the catalog-derived canonical component suffix (currently `*.kajovocml.hcasc.cz`). Before certificate issuance, the installer validates and reloads the rendered nginx configuration so the short-lived DNS-01 challenge is available at `http://admin.<PUBLIC_BASE_DOMAIN>/.well-known/acme-challenge/kcml-dns-challenge.json`. If coverage is missing or has less than 30 days remaining, the root-only release installer requests a replacement certificate in a dedicated process group with a root-only PID file; an interrupted prior KCML certificate request is terminated before retry without affecting unrelated certbot jobs. DNS propagation is checked directly against every authoritative nameserver so a cached negative answer on the production resolver cannot block issuance. The private key is generated and retained on the production server. A failure restores the previous nginx and service configuration; preflight then verifies both SANs before release activation.
...
- During onboarding, callers use the integration bearer only for the explicitly granted secret names recorded on that token. After automatic activation, the same Secret API endpoint accepts the component's long-lived KCML access bearer with the `secret.resolve` scope and the migrated grant set bound to the component principal.
