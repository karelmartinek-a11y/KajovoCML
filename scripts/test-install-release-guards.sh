...
grep -Fq 'install -d -m 0755 -o kcml-deploy -g kcml-deploy /opt/actions-runner/kcml-deploy/_work/_temp' "$install_script"
grep -Fq 'chown -R kcml-deploy:kcml-deploy /opt/actions-runner/kcml-deploy/_work/_temp' "$install_script"
...
grep -Fq 'wait_for_sql_equals "baseline_migration_count" "5" "select count(*) from schema_migration"' "$install_script"
