...
const schema = {
  ...
  properties: {
    ...
    runtime: { type: "object", required: ["transport", "executionMode", "runtimeDigest", "resources", "lifecycle", "readinessMode", "persistentState", "secretGrants", "egressGrants"], additionalProperties: false, properties: { transport: { enum: ["UDS", "HTTPS"] }, executionMode: { enum: ["REQUEST_RESPONSE", "LONG_RUNNING"] }, runtimeDigest: sha, upstream: { type: "string", format: "uri" }, socketPath: { type: "string", pattern: "^/" }, tlsIdentity: { type: "string" }, container: { type: "object" }, lifecycle: { type: "object", required: ["prepareRequired", "gracefulShutdownSeconds", "singleActiveWorker"], additionalProperties: false, properties: { prepareRequired: { type: "boolean" }, gracefulShutdownSeconds: { type: "integer", minimum: 1, maximum: 600 }, singleActiveWorker: { type: "boolean" } } }, readinessMode: { const: "DEPENDENCY_AWARE" }, persistentState: { type: "object", required: ["required", "mountPath", "survivesRestart", "survivesUpgrade", "survivesRollback"], additionalProperties: false, properties: { required: { type: "boolean" }, mountPath: { type: "string", pattern: "^/" }, survivesRestart: { const: true }, survivesUpgrade: { const: true }, survivesRollback: { const: true } } }, secretGrants: { type: "array", items: runtimeSecretGrant }, egressGrants: { type: "array", items: runtimeEgressGrant }, resources: { type: "object", required: ["cpuMillis", "memoryMiB", "maxConcurrency"], additionalProperties: false, properties: { cpuMillis: { type: "integer", minimum: 10 }, memoryMiB: { type: "integer", minimum: 16 }, maxConcurrency: { type: "integer", minimum: 1 } } } } },
    ...
    outboundPolicies: { type: "array", items: runtimeEgressGrant },
...
const example = {
  ...
  runtime: { transport: "HTTPS", executionMode: "REQUEST_RESPONSE", runtimeDigest: digest("inventory-runtime-v1"), upstream: "https://inventory-runtime.example.com", tlsIdentity: "inventory-runtime.example.com", lifecycle: { prepareRequired: false, gracefulShutdownSeconds: 30, singleActiveWorker: false }, readinessMode: "DEPENDENCY_AWARE", persistentState: { required: false, mountPath: "/var/lib/kcml-data", survivesRestart: true, survivesUpgrade: true, survivesRollback: true }, secretGrants: [], egressGrants: [{ type: "HTTPS_FETCH", targetHost: "inventory-runtime.example.com", port: 443, pathPrefix: "/", scope: "inventory.lookup" }], resources: { cpuMillis: 500, memoryMiB: 256, maxConcurrency: 20 } },
  ...
  secretPolicy: { authorizationAuthority: "KCML", allSecretsRequireGrant: true, auditLevel: "FULL" }, outboundPolicies: [{ type: "HTTPS_FETCH", targetHost: "inventory-runtime.example.com", port: 443, pathPrefix: "/", scope: "inventory.lookup" }],
