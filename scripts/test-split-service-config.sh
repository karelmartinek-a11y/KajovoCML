...
DATABASE_APP_URL=postgres://kcml_app:stale@127.0.0.1/kcml
...
ENV
DATABASE_APP_URL=postgres://kcml_app:rotated@127.0.0.1/kcml \
  KCML_LEGACY_ENV="$tmp/legacy.env" KCML_CONFIG_ROOT="$tmp/config" \
...
test "$(cat "$tmp/config/credentials/web/database_url")" = 'postgres://kcml_app:rotated@127.0.0.1/kcml'
test "$(cat "$tmp/config/credentials/worker/database_url")" = 'postgres://kcml_app:rotated@127.0.0.1/kcml'
