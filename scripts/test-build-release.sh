...
grep -q 'podman_runtime_dir="/run/user/${kcml_uid}"' "$tmpdir/release/deploy/scripts/install-release.sh"
grep -q 'DBUS_SESSION_BUS_ADDRESS="unix:path=${podman_runtime_dir}/bus"' "$tmpdir/release/deploy/scripts/install-release.sh"
