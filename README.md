...
AI agents, MCP-facing components and deterministic microsteps may be maintained outside KajovoCML and then follow the current generic component onboarding catalog at registration time. When they are maintained in this monorepository, they belong exclusively in `components/<repository-key>/`; their source layout and generation flow are currently governed by the active test contract `docs/onboarding-catalogs/repository-component-1.1.json`, which is now the current test source catalog for both request-response and stateful `LONG_RUNNING` repository components, while runtime registration remains governed by the current component catalog and `/v2/component-onboardings`. Integration tokens authorize KCML registration only and must never be committed or used as GitHub or deployment credentials.
