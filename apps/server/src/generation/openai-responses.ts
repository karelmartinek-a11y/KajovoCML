...
export async function implementGeneration(apiKey: string, model: string, input: {
  ...
  inputPayload = `FINAL_CANDIDATE_VALIDATION_FAILED. Výsledek nelze přijmout. Znovu načti component-manifest.example.json, zachovej jeho přesnou povolenou strukturu a oprav uvedené soubory. Každý handler.mjs MUSÍ obsahovat doslovně pojmenované ESM exporty: export const tools = [ ... ] (pole tool objektů) a export async function invoke(name, input, context) { ... }. Default export, pouze lokální const bez exportu ani jiný tvar nenahrazuje tools/invoke. Znovu zavolej validate_candidate_artifacts pro každý pár a vrať finální JSON až po validním výsledku. Bezpečné validační chyby:\n${JSON.stringify(validations)}`;
        continue;
      }
