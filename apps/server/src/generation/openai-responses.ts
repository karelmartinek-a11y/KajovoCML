...
export async function planGeneration(apiKey: string, model: string, prompt: string, signal?: AbortSignal): Promise<GenerationPlan> {
  const instructions = `Jsi interní intent/spec planner KajovoCML. Vrať pouze validní JSON bez markdownu. Plánuj minimální počet samostatných CML prvků. Každý prvek je MCP_SERVER nebo AI_AGENT. AI_AGENT použij jen když je skutečně nutné LLM reasoning/generování. Neplánuj GitHub, pull requesty, CI, GHCR, OCI, integration token ani externí programmer handoff. Externí provider webhook je normální CML endpoint pod hostname výsledného prvku a provider-specific ověření se deklaruje v endpoint.auth. Ownera se ptej jen na skutečně chybějící credential, účet, URL, telefon, e-mail nebo business pravidlo. Secret input označ secret=true a přiděl stableSecretName v uppercase. Secret, který má vydat provider až během post-deploy integrace, uveď v requiredSecretNames i providerGeneratedSecretNames a NEVYŽADUJ jej od OWNERa jako missingInput. Každá dependency musí uvést sourceTool: název nástroje zdrojového prvku, jehož skutečné provedení tuto dependency použije, a targetTools: přesné názvy nástrojů cílového prvku; následná implementace je musí opravdu vytvořit a sourceTool musí přes context.callComponent volat cílový prvek. JSON shape: {"understoodIntent":string,"resultSummary":string,"elements":[{"key":string,"kind":"MCP_SERVER"|"AI_AGENT","displayName":string,"businessPurpose":string,"responsibilities":string[],"requiredSecretNames":string[],"providerGeneratedSecretNames"?:string[]}],"dependencies":[{"from":string,"to":string,"purpose":string,"sourceTool":string,"targetTools":string[]} ],"missingInputs":[{"key":string,"label":string,"description":string,"kind":"TEXT"|"URL"|"EMAIL"|"PHONE"|"PASSWORD"|"API_KEY"|"SECRET"|"RULE","required":boolean,"secret":boolean,"stableSecretName"?:string,"grantToElementKeys"?:string[]}]} .`;
...
export async function implementGeneration(apiKey: string, model: string, input: {
  ...
  const instructions = `Jsi zaměřený interní implementátor KajovoCML. Pracuješ výhradně v přiděleném workspace. docs/SSOT_CURRENT.md a component-manifest-2026.07.22-compliance.1.schema.json jsou závazné. IMPLEMENTING vytváří zdroj, manifest a integrační plán; NESMÍ konfigurovat externí provider ani předpokládat živý callback. Web research je dovolen. Nevytvářej mock, placeholder, TODO ani demo a nepoužívej GitHub/CI/GHCR/OCI/integration token. Handler exportuje tools a async invoke(name,input,context); side-effecty jsou pouze context.secret, context.callComponent, context.callExternal a bounded context.state. context.callExternal podporuje {targetHost,routePath,scope,method,headers,bodyType,body,payload,timeoutMs}; metody GET/POST/PUT/PATCH/DELETE/HEAD musí jít výhradně přes CML. Pro outbound policy HTTPS_FETCH deklaruj port 443, targetHost, pathPrefix, scope, volitelné methods a auth NONE/BEARER_SECRET/HEADER_SECRET. Veřejný provider webhook deklaruj pod /webhooks/... s EXTERNAL_WEBHOOK verification. Secret, který má provider vydat až po nasazení candidate, zůstává v element.requiredSecretNames a providerGeneratedSecretNames a nesmí být hardcoded. Pro každý element vytvoř/uprav elements/<key>/handler.mjs a manifest.kcml.json, proveď lokální syntax/test a vrať pouze JSON {"summary":string,"elements":[{"key":string,"handlerPath":string,"manifestPath":string}],"integrationPlan":{"required":boolean,"summary":string,"steps":string[]}} nebo needsInput pro skutečně chybějící OWNER údaj. Provider-side konfigurace proběhne až v samostatné INTEGRATING fázi po deploy candidate runtime.`
