...
async function main(): Promise<void> {
  ...
  try {
    ...
    const prompt = [
      ...
      "Použij bezpečný reference-api readiness fixture, pokud je to vhodné; žádný produkční write, žádný secret a žádný OWNER follow-up není potřeba.",
    ...
    if (jobId) {
      ...
      await check("Responses discussion and capability-first evidence", async () => {
        ...
        return { state: string(job.state), currentSpec: Boolean(job.currentSpecRevisionId), lookup: hasLookup, contractInspection: hasContract, proposal: hasProposal, assistantMessages: assistant.length };
      });
      ...
      await check("generation cancellation is idempotent", async () => {
        await client.json(`/api/generation/jobs/${jobId}/cancel`, { method: "POST", headers: { "content-type": "application/json" }, body: "{}" }, [200, 202]);
        await client.json(`/api/generation/jobs/${jobId}/cancel`, { method: "POST", headers: { "content-type": "application/json" }, body: "{}" }, [200, 202, 409]);
        const job = await waitForJob(client, jobId, (candidate) => string(candidate.state) === "CANCELLED", 30_000);
    ...
    if (definitionId) {
      ...
      await check("Playwright preflight and runtime verification", async () => {
        ...
        await client.json(`/api/browser-automations/${definitionId}/preflight`, { method: "POST", headers: { "content-type": "application/json" }, body: "{}" });
        return { revisionId, verification: "PASS", preflight: "PASS" };
      });
    ...
    await check("safe production acceptance cleanup", async () => {
      ...
      return { automationFixtures: createdAutomations.length, discussionFixtures: createdJobs.length, cleanup: "attempted" };
    });
