...
describe.skipIf(!enabled)("MCP concurrency PostgreSQL lease", () => {
  ...
  it("allows exactly one of two concurrent acquisitions at maxConcurrency one", async () => {
    const inserted = await db.query(
      `insert into mcp_server(kcml_number,code,hostname,tool_name,display_name,description,enabled,registration_state,operational_state,input_schema,output_schema,handler_key,handler_version,contract_version,artifact_digest,manifest_digest,max_concurrency,timeout_ms)
       values (9002,'KCML9002','kcml9002.example.invalid','lease_test','Lease','Lease',true,'ACTIVE','HEALTHY','{}','{}','test','1','1',$1,$2,1,1000) returning id`,
