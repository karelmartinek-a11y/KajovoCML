...
describe.skipIf(!enabled)("canonical OpenAI Secret Manager readiness", () => {
  beforeAll(async () => {
    ...
    db = createDb(config);
    const actor = await db.query("select id from admin_account order by created_at limit 1");
    if (!actor.rowCount) throw new Error("generation_openai_readiness_actor_missing");
  ...
  it("reuses the existing canonical secret, reconciles only its direct PLATFORM grant, and fails closed for unusable states", async () => {
    ...
    const secret = await createSecret(db, config, actorId, randomUUID(), {
    ...
    const reconciled = await reconcileGenerationOpenAiReadiness(db, config, actorId, randomUUID());
    ...
    const disabled = await setSecretStatus(db, actorId, randomUUID(), secret.id, secret.lockVersion, "DISABLED");
    expect((await generationOpenAiReadiness(db, config)).reason).toBe("INACTIVE");
    const active = await setSecretStatus(db, actorId, randomUUID(), secret.id, disabled.lockVersion, "ACTIVE");
    ...
    await deleteSecret(db, actorId, randomUUID(), secret.id, active.lockVersion);
