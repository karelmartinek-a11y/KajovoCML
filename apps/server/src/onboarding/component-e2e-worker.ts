...
type Fixture = {
  ...
  generated_endpoint_path?: string; generated_endpoint_method?: string; generated_pulse_direction?: string;
};

const digest = (value: Buffer) => `sha256:${createHash("sha256").update(value).digest("hex")}`;
const ajv = new Ajv2020({ strict: false, allErrors: true, validateFormats: false });

function udsPost(socketPath: string, path: string, body: Buffer, mediaType: string, timeoutMs: number, token: string): Promise<{ status: number; body: Buffer }> {
  return new Promise((resolve, reject) => {
    const request = http.request({ socketPath, path, method: "POST", timeout: timeoutMs,
      headers: { authorization: `Bearer ${token}`, "content-type": mediaType, "content-length": body.length, "x-kcml-platform-operation": "e2e" } }, (response) => {
      const chunks: Buffer[] = [];
...
async function claim(db: Db, workerId: string): Promise<E2ERun | null> {
  return tx(db, async (client) => {
    ...
    const result = await client.query(
      `select run.*,target.transport,target.upstream,target.expected_tls_identity,target.socket_path,component.hostname,component.registration_type,cri.stable_secret_name as runtime_secret_name,revision.manifest_digest as revision_digest
         ...
         left join component_runtime_identity cri on cri.component_id=run.component_id
...
function invocationRequest(run: E2ERun, fixture: Fixture, cleanup: boolean): { path: string; mediaType: string; body: Buffer } {
  ...
  if (fixture.invocation_kind === "TOOL") {
    ...
    if (run.registration_type === "INTERNAL_GENERATED") return {
      path: "/mcp",
      mediaType: "application/json",
      body: Buffer.from(JSON.stringify({ jsonrpc: "2.0", id: `${fixture.scenario_key}:${fixture.variant_key}`, method: "tools/call", params: { name: fixture.invocation_name, arguments: JSON.parse(fixture.input_content.toString("utf8")) } }))
    };
    return {
  ...
  if (fixture.invocation_kind === "PULSE") {
    if (run.registration_type === "INTERNAL_GENERATED") return {
      path: "/v1/kcml/runtime/pulse",
      mediaType: "application/json",
      body: Buffer.from(JSON.stringify({ direction: fixture.generated_pulse_direction ?? "INCOMING", pulseType: fixture.invocation_name, payload: JSON.parse(fixture.input_content.toString("utf8")) }))
    };
    return { path: "/v1/kcml/runtime/pulse", mediaType: fixture.input_media_type, body: fixture.input_content };
  }
  if (fixture.invocation_kind === "ENDPOINT") {
    if (run.registration_type === "INTERNAL_GENERATED") return {
      path: "/v1/kcml/runtime/endpoint",
      mediaType: "application/json",
      body: Buffer.from(JSON.stringify({ endpointKey: fixture.invocation_name, request: { method: fixture.generated_endpoint_method ?? "POST", path: fixture.generated_endpoint_path ?? fixture.invocation_name, body: JSON.parse(fixture.input_content.toString("utf8")) } }))
    };
    return { path: fixture.invocation_name, mediaType: fixture.input_media_type, body: fixture.input_content };
  }
  throw new Error("e2e_invocation_kind_unsupported");
}

async function invoke(config: WorkerConfig, run: E2ERun, fixture: Fixture, token: string, cleanup = false): Promise<{ status: number; body: Buffer }> {
  ...
  if (run.transport === "UDS") {
    ...
    return udsPost(run.socket_path, invocation.path, invocation.body, invocation.mediaType, Number(fixture.timeout_ms), token);
  }
  ...
  const response = await fetchThroughEgress(config, { url: new URL(invocation.path, upstream).toString(), method: "POST",
    headers: { authorization: `Bearer ${token}`, "content-type": invocation.mediaType, "x-kcml-platform-operation": "e2e", "x-kcml-target-hostname": run.hostname }, body: invocation.body,
...
function normalizedResponse(body: Buffer, mediaType: string, run: E2ERun, fixture: Fixture): Buffer {
  if (mediaType !== "application/json") return body;
  const parsed = JSON.parse(body.toString("utf8"));
  if (run.registration_type === "INTERNAL_GENERATED" && fixture.invocation_kind === "TOOL") {
    return Buffer.from(canonicalJson(parsed?.result?.structuredContent));
  }
  if (run.registration_type === "INTERNAL_GENERATED" && fixture.invocation_kind === "ENDPOINT") return Buffer.from(canonicalJson(parsed?.body));
  if (run.registration_type === "INTERNAL_GENERATED" && fixture.invocation_kind === "PULSE") return Buffer.from(canonicalJson(parsed?.result));
...
export async function processNextComponentE2ERun(db: Db, config: WorkerConfig, workerId: string): Promise<boolean> {
  ...
  const toolContracts = await db.query("select name,scope_name,output_schema from component_tool_contract where component_id=$1 and revision_id=$2 order by name", [run.component_id, run.revision_id]);
  const endpointContracts = await db.query("select endpoint_id,path,methods from component_endpoint_contract where component_id=$1 and revision_id=$2 order by endpoint_id", [run.component_id, run.revision_id]);
  ...
  if (run.registration_type === "INTERNAL_GENERATED") {
    if (!run.runtime_secret_name) throw new Error("generated_runtime_identity_missing");
    const principal = await platformWorkerSecretPrincipal(db);
    runtimeToken = (await resolveSecret(db, config, principal, run.runtime_secret_name, run.correlation_id)).value;
  }
  let passed = Number(fixtures.rowCount ?? 0) > 0;
  const endpointById = new Map(endpointContracts.rows.map((contract) => [String(contract.endpoint_id), contract]));
  const pulseByName = new Map(pulseContracts.rows.map((contract) => [String(contract.pulse_type), contract]));
  const gateEvidence: Array<Record<string, unknown>> = [];
  for (const raw of fixtures.rows as Fixture[]) {
    if (run.registration_type === "INTERNAL_GENERATED" && raw.invocation_kind === "ENDPOINT") {
      const contract = endpointById.get(raw.invocation_name);
      raw.generated_endpoint_path = contract ? String(contract.path) : raw.invocation_name;
      raw.generated_endpoint_method = Array.isArray(contract?.methods) && contract.methods.length ? String(contract.methods[0]) : "POST";
    }
    if (run.registration_type === "INTERNAL_GENERATED" && raw.invocation_kind === "PULSE") raw.generated_pulse_direction = String(pulseByName.get(raw.invocation_name)?.direction ?? "INCOMING");
    ...
    try {
      ...
      if (runtimeToken && raw.invocation_kind === "TOOL") {
        const contract = toolContracts.rows.find((candidate) => String(candidate.name) === raw.invocation_name);
        const componentDecision = await authorizeComponentCall(db, {
          token: runtimeToken, audience: `https://${run.hostname}`, host: run.hostname,
          scope: String(contract?.scope_name ?? "mcp.tools.call"), route: `/mcp/tools/${raw.invocation_name}`,
          hmacKey: config.ACCESS_TOKEN_HMAC_KEY_BASE64, correlationId: run.correlation_id
        });
      ...
      const response = await invoke(config, run, raw, token);
      if (response.status < 200 || response.status >= 300) throw new Error(`e2e_http_${response.status}`);
      responseContent = normalizedResponse(response.body, raw.expected_media_type, run, raw);
      ...
      if (raw.cleanup_contract?.required) {
        const cleanup = await invoke(config, run, raw, token, true);
  ...
  for (const gate of gates) {
    ...
    ) values ($1,$2,$3,$4,$5,$6,$7::jsonb,$8,$9,$10,$11,$12,$12,$13,$14,$15)
    ...
    [run.component_id, run.revision_id, gate.gate, KCML_RELEASE.catalogVersion, gate.pass ? "PASS" : "FAIL",
      ...
      // Exact E2E evidence is bound to the immutable active revision and runtime
      // digests. Runtime changes invalidate it through those bindings; applying a
      // wall-clock expiry would make an already-handed-off component unready even
      // though its onboarding-only scenario cannot safely be replayed.
      run.correlation_id, null, run.revision_digest, run.runtime_digest, requestDigest, responseDigest, gate.variant]);
  }
  return true;
}
