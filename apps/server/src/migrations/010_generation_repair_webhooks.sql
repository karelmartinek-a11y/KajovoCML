...
ALTER TABLE component_endpoint_contract
...
ALTER TABLE component_external_permission
...
ALTER TABLE generation_job
  ...
  ADD COLUMN repair_component_id uuid REFERENCES component(id) ON DELETE SET NULL,
  ADD COLUMN repair_evidence jsonb,
  ADD COLUMN repair_base_release_id uuid REFERENCES local_component_release(id) ON DELETE SET NULL,
...
CREATE UNIQUE INDEX generation_job_active_repair_component_idx
  ON generation_job(repair_component_id)
  WHERE job_kind='REPAIR' AND state NOT IN ('COMPLETED','FAILED','BLOCKED','CANCELLED') AND repair_component_id IS NOT NULL;

ALTER TABLE local_component_release
  ADD COLUMN generation_job_id uuid REFERENCES generation_job(id) ON DELETE SET NULL,
...
ALTER TABLE generation_component DROP CONSTRAINT IF EXISTS generation_component_component_id_key;
CREATE INDEX generation_component_component_idx ON generation_component(component_id,created_at DESC);
...
UPDATE principal_access_token token
   SET target_component_id=null,
       audience='*'
  FROM component_runtime_identity identity
  JOIN component generated_component ON generated_component.id=identity.component_id
