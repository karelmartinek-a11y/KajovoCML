...
import { accessLabels, type AccessTokenCredential, type AccessTokenPermission, type Component, type ComponentAccessToken, type SecretResult, type Server } from "./types.js";
...
type ComponentTokenRow = { component: Component; token: ComponentAccessToken };

export function CredentialsPage({ credentials, components, onOpenCreate, onEditPermissions, onRename, onConfirm, onComponentTokenRevoke, onComponentTokenRotate, onRefresh }: { credentials: AccessTokenCredential[]; components: Component[]; onOpenCreate: () => void; onEditPermissions: (id: string) => void; onRename: (credential: AccessTokenCredential) => void; onConfirm: (credential: AccessTokenCredential, action: "revoke" | "delete") => void; onComponentTokenRevoke?: (component: Component, token: ComponentAccessToken) => Promise<void>; onComponentTokenRotate?: (component: Component, token: ComponentAccessToken) => Promise<{ token: string; fingerprint: string }>; onRefresh: () => void }) {
  const [query, setQuery] = useState("");
  const [revealedComponentToken, setRevealedComponentToken] = useState<{ component: Component; token: string; fingerprint: string } | null>(null);
  const filtered = credentials.filter((credential) => `${credential.label} ${credential.publicId}`.toLowerCase().includes(query.toLowerCase()));
  const componentTokens: ComponentTokenRow[] = components.flatMap((component) => (component.accessTokens ?? []).map((token) => ({ component, token })));
  return (
    <>
      ...
      <section className="panel table-panel"><div className="panel-head"><div><h2>Dlouhodobé přístupové tokeny komponent</h2><p>Tyto tokeny jsou identitou komponenty. Oprávnění jsou jejich deklarované scope; onboardingový token sem nepatří.</p></div><span className="panel-count">{componentTokens.length} záznamů</span></div>
        {componentTokens.length === 0 ? <div className="empty-state"><KeyRound size={34} /><strong>Žádné tokeny komponent k zobrazení</strong></div> : <table><thead><tr><th>Komponenta</th><th>Fingerprint</th><th>Oprávnění (scope)</th><th>Stav</th><th>Vydáno</th><th>Naposledy použito</th>{onComponentTokenRotate && onComponentTokenRevoke ? <th>Akce</th> : null}</tr></thead><tbody>{componentTokens.map(({ component, token }) => <tr key={token.id}><td><strong>{component.displayName}</strong><span className="cell-subtitle">{component.code}</span></td><td><code>{token.fingerprint}</code></td><td>{token.scope_names.join(", ") || "Bez scope"}</td><td><span className={`badge ${token.revoked_at ? "danger" : "ok"}`}>{token.revoked_at ? "Revokován" : "Aktivní"}</span></td><td>{formatDate(token.issued_at)}</td><td>{formatDate(token.last_used_at)}</td>{onComponentTokenRotate && onComponentTokenRevoke ? <td><div className="row-actions"><button className="small-button" disabled={Boolean(token.revoked_at)} onClick={() => { if (window.confirm(`Rotovat dlouhodobý přístupový token komponenty ${component.code}? Dosavadní token bude okamžitě revokován.`)) void onComponentTokenRotate(component, token).then((result) => setRevealedComponentToken({ component, ...result })); }}>Rotovat</button><button className="small-button danger-link" disabled={Boolean(token.revoked_at)} onClick={() => { if (window.confirm(`Nevratně revokovat dlouhodobý přístupový token komponenty ${component.code}?`)) void onComponentTokenRevoke(component, token); }}>Revokovat</button></div></td> : null}</tr>)}</tbody></table>}
      </section>
      {revealedComponentToken ? <Modal title="Nový přístupový token komponenty" onClose={() => setRevealedComponentToken(null)}><div className="secret-dialog"><p>Nová hodnota pro {revealedComponentToken.component.code} se zobrazí pouze nyní. Bezpečně ji předej běžící komponentě.</p><div className="secret-once"><code>{revealedComponentToken.token}</code><span>Fingerprint {revealedComponentToken.fingerprint}.</span></div><footer className="modal-actions"><button onClick={() => setRevealedComponentToken(null)}>Rozumím, skrýt token</button></footer></div></Modal> : null}
    </>
  );
}
