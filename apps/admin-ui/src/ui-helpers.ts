...
export function describeApiError(code: string, correlationId: string | null = null): string {
  const map: Record<string, string> = {
    ...
    client_id_mismatch: "Deklarovaný Client ID neodpovídá autentizovanému credentialu komponenty.",
    source_component_mismatch: "Deklarovaný zdroj komponenty neodpovídá tokenu.",
    target_component_mismatch: "Deklarovaný cíl komponenty neodpovídá kanonickému hostu.",
    component_identity_required: "Příchozí volání musí explicitně deklarovat Client ID a komponentovou identitu.",
    component_identity_mismatch: "Deklarovaná komponentová identita neodpovídá aktivní revizi nebo hostu.",
    ...
    policy_epoch_mismatch: "Požadavek používá zastaralý policy epoch a byl fail-closed odmítnut.",
    heartbeat_clock_skew: "Heartbeat má nepřípustný časový posun a nebyl přijat jako platný důkaz.",
    heartbeat_challenge_mismatch: "Heartbeat neodpovídá očekávané challenge nebo nonce.",
    audit_stream_conflict: "Auditní stream komponenty je v konfliktu a komponenta byla fail-closed umístěna do karantény.",
