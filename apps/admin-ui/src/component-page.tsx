...
import type { Component, PlatformWorkerAccessStatus } from "./types.js";
...
function ComponentDetail({ component, busy, onClose, onToggle, onLifecycle, onPermission, onAccessTokenRevoke, onAccessTokenRotate, onRunE2E, onRunStateQuery, onRunHeartbeatChallenge }: {
  component: Component;
  busy: boolean;
  onClose: () => void;
  ...
  return <Modal title={`${component.code} · ${component.displayName}`} onClose={onClose} className="component-detail-modal">
    ...
    <section className="component-detail-section"><h3>Příchozí a odchozí oprávnění</h3>{component.permissions?.length ? <div className="table-scroll"><table><thead><tr><th>Směr</th><th>Scope</th><th>Route</th><th>Stav</th>{showOwnerActions ? <th>Akce</th> : null}</tr></thead><tbody>{component.permissions.map((permission) => <tr key={permission.id}><td>{permission.target_component_id === component.id ? "Příchozí" : "Odchozí"}</td><td>{permission.scope_name}</td><td><code>{permission.route_pattern}</code></td><td>{permission.revoked_at ? "Revokováno" : "Platné"}</td>{showOwnerActions ? <td><button className="secondary compact-button" disabled={busy} onClick={() => confirmed(permission.revoked_at ? "Obnovit toto směrované oprávnění?" : "Odebrat toto směrované oprávnění? Aktivní volání budou okamžitě odmítnuta.", () => { void onPermission(component, permission.id, Boolean(permission.revoked_at)); })}>{permission.revoked_at ? "Obnovit" : "Odebrat"}</button></td> : null}</tr>)}</tbody></table></div> : <p>Komponenta nemá směrovaná oprávnění.</p>}</section>
    <section className="component-detail-section"><h3>Runtime identita prvku</h3>{component.accessTokens?.filter((token) => !token.revoked_at).length ? <div className="table-scroll"><table><thead><tr><th>Druh</th><th>Fingerprint</th><th>Scope</th><th>Vydáno</th><th>Naposledy použito</th>{showOwnerActions ? <th>Akce</th> : null}</tr></thead><tbody>{component.accessTokens.filter((accessToken) => !accessToken.revoked_at).map((accessToken) => <tr key={accessToken.id}><td>Dlouhodobý runtime token</td><td><code>{accessToken.fingerprint}</code></td><td>{accessToken.scope_names.join(", ")}</td><td>{formatDate(accessToken.issued_at)}</td><td>{formatDate(accessToken.last_used_at)}</td>{showOwnerActions ? <td><div className="inline-actions"><button className="secondary compact-button" disabled={busy} onClick={() => confirmed("Rotovat tento dlouhodobý runtime token? Současný token bude okamžitě revokován a KajovoCML přepne komponentu na novou hodnotu.", () => { void onAccessTokenRotate(component, accessToken.id).then(setRevealedToken); })}>Rotovat</button><button className="danger-button compact-button" disabled={busy} onClick={() => confirmed("Nevratně revokovat tento dlouhodobý runtime token? Komponenta bude okamžitě zablokována.", () => { void onAccessTokenRevoke(component, accessToken.id); })}>Revokovat</button></div></td> : null}</tr>)}</tbody></table></div> : <p>Prvek právě nemá platnou dlouhodobou runtime identitu.</p>}</section>
...
export function ComponentCatalogPage({ components, platformWorkerAccess, onRefresh, onLoadDetail, onToggle, onLifecycle, onPermission, onAccessTokenRevoke, onAccessTokenRotate, onRotatePlatformWorkerAccess, onRunE2E, onRunStateQuery, onRunHeartbeatChallenge }: {
  ...
  return <>
    ...
    {selected ? <ComponentDetail component={selected} busy={busy} onClose={() => setSelected(null)} onToggle={toggle} onLifecycle={runLifecycle} onPermission={permission} onAccessTokenRevoke={revokeAccessToken} onAccessTokenRotate={rotateAccessToken} onRunE2E={onRunE2E} onRunStateQuery={onRunStateQuery} onRunHeartbeatChallenge={onRunHeartbeatChallenge} /> : null}
