...
function ComponentDetail({ component, role, busy, onClose, onToggle, onLifecycle, onPermission, onAccessTokenRevoke, onAccessTokenRotate, onRunE2E, onRunStateQuery, onRunHeartbeatChallenge }: {
  ...
  return <Modal title={`${component.code} · ${component.displayName}`} onClose={onClose} className="component-detail-modal">
    ...
    <section className="component-detail-section"><h3>Aktuální token prvku</h3>{component.currentOnboardingToken ? <dl><dt>Druh</dt><dd>Integrační onboardingový token</dd><dt>Označení</dt><dd>{component.currentOnboardingToken.label}</dd><dt>Fingerprint</dt><dd><code>{component.currentOnboardingToken.fingerprint}</code></dd><dt>Platnost do</dt><dd>{formatDate(component.currentOnboardingToken.expires_at)}</dd><dt>Stav onboardingu</dt><dd>{component.currentOnboardingToken.state}</dd></dl> : component.accessTokens?.filter((token) => !token.revoked_at).length ? <div className="table-scroll"><table><thead><tr><th>Druh</th><th>Fingerprint</th><th>Scope</th><th>Vydáno</th><th>Naposledy použito</th>{role !== "AUDITOR" ? <th>Akce</th> : null}</tr></thead><tbody>{component.accessTokens.filter((accessToken) => !accessToken.revoked_at).map((accessToken) => <tr key={accessToken.id}><td>Dlouhodobý přístupový token</td><td><code>{accessToken.fingerprint}</code></td><td>{accessToken.scope_names.join(", ")}</td><td>{formatDate(accessToken.issued_at)}</td><td>{formatDate(accessToken.last_used_at)}</td>{role !== "AUDITOR" ? <td><div className="inline-actions"><button className="secondary compact-button" disabled={busy} onClick={() => confirmed("Rotovat tento dlouhodobý přístupový token? Současný token bude okamžitě revokován a nový se zobrazí pouze jednou.", () => { void onAccessTokenRotate(component, accessToken.id).then(setRevealedToken); })}>Rotovat</button><button className="danger-button compact-button" disabled={busy} onClick={() => confirmed("Nevratně revokovat tento dlouhodobý přístupový token?", () => { void onAccessTokenRevoke(component, accessToken.id); })}>Revokovat</button></div></td> : null}</tr>)}</tbody></table></div> : <p>Prvek právě nemá platný onboardingový ani dlouhodobý přístupový token.</p>}</section>
