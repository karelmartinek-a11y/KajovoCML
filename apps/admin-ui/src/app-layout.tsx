  ...
  return (
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <nav>
          {navigationButton("dashboard", "Dashboard", <LayoutDashboard size={18} />)}
          {navigationButton("generation", "Generování", <Sparkles size={18} />)}
          {navigationButton("automations", "Browser automatizace", <Workflow size={18} />)}
          {navigationButton("registered", "Registrované prvky", <Activity size={18} />)}
          ...
          {navigationButton("identities", "Tokeny a identity", <KeyRound size={18} />)}
          {navigationButton("secrets", "Secrets", <Lock size={18} />)}
          {navigationButton("permissions", "Správa oprávnění", <LockKeyhole size={18} />)}
          {navigationButton("audit", "Audit", <Terminal size={18} />)}
          {navigationButton("config", "Konfigurace", <SlidersHorizontal size={18} />)}
          {navigationButton("security", "Bezpečnost", <ShieldCheck size={18} />)}
          {navigationButton("admins", "OWNER účty", <Plus size={18} />)}
        </nav>
