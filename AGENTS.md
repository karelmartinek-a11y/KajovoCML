...
- A model's final implementation JSON is not acceptance evidence. The server must re-read and validate every declared manifest/handler pair after the final response and keep the bounded Responses loop open for correction when that last content is invalid.
...
Full authenticated acceptance runs only through the separate
`.github/workflows/production-acceptance.yml` workflow with an exact already
deployed SHA. It performs no release build, migration, release switch, WEDOS
mutation or TLS issuance; ordinary deploy remains a bounded release-critical
fast path.
